...
const container = document.querySelector(
  "#quiz-box"
); /* container variable is assigned for quiz box */
  ...
  if (counter > questions.length - 1) {
    ...
    quizContainer.innerHTML = `
            <div class="question-container">
                ...
                <ul class="answers">
                    <li class="answer" data-option="${letters[0]}">${
      questions[counter][letters[0]]
    }</li>
                    <li class="answer" data-option="${letters[1]}">${
      questions[counter][letters[1]]
    }</li>
                    <li class="answer" data-option="${letters[2]}">${
      questions[counter][letters[2]]
    }</li>
                    <li class="answer" data-option="${letters[3]}">${
      questions[counter][letters[3]]
    }</li>
                </ul>
            </div>
        `;
  ...
  if (inputUsername.value) {
    ...
    if (scorePoint == fullScorePoint) {
      ...
      if (scorePoint > oldScore) {
        ...
        msgGreeting.innerText =
          "Try harder! Your previous score was better than this!";
      }
    } else {
...
function createTableRow(rank, name, score) {
  ...
  if (rank == 1) rank = `<span class="emoji">🥇</span>`;
  else if (rank == 2) rank = `<span class="emoji">🥈</span>`;
  else if (rank == 3) rank = `<span class="emoji">🥉</span>`;
  tableRow.innerHTML = ` <h1>${rank}</h1>
                        <h1>${name}</h1>
                        <h1>0</h1>`;
  if (currentUser) {
    if (
      tableRow.firstElementChild.nextElementSibling.innerText == currentUser
    ) {
      if (score >= oldScore) {
        count = oldScore;
      }
